...
Using a simple algroithm that evaluates $P(W \geq 1)$ with $m = 365$ and increments $n$ from 1 up until the probability we are searching for is exceeded.\\
	\begin{algorithm}
    \begin{algorithmic}
	\Procedure{tripleBirthday}{number of days : $m$, probabilty threshold : $P_{\text{thresh}}$}
		\State$n \gets 0$
		\While{$P_n < P_{\text{thresh}}$}
			\State $n \gets n + 1$
			\State sum $\gets 0$
			\For{$i \gets 1, i < n/2, i \gets i + 1$}
